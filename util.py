...
def connect_to_db(username='me', password='test', host='127.0.0.1', port='5432', database='KybPartPickerInventory'):
    try:
...
def fetch_case(cursor, sql_string="select case_name from product_case;"):
    try:
...
# run_sql(cursor,"select from;")
def run_and_fetch_sql(cursor, sql_string="select * from product_case"):
    try:
        # Executing a SQL query
        # cursor.execute("SELECT version();")
        # cursor.execute("SELECT * from customer;")
